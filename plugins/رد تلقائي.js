...
handler.all = async function (m) {
    ...
    if (/^السلام عليكم|السلام|سلام|السلام عليكم ورحمة الله وبركاته+/i.test(m.text)) {
    ...
    } else if (/^كيف حالك|كيفك$/i.test(m.text)) {
        responses = ['الحمد لله كيفك انت','بخير وانت',];
    } else if (/^منور|نورت$/i.test(m.text)) {
        responses = ['*ادري بنوري انا*','النور نورك يسطاء',];
} else if (/^بوت|الرعب الملكي$/i.test(m.text)) {
        responses = ['*اكتب نقطه وكلمة اوامر لإظهار الامر*','اذا واجهتك مشكله تعال كلمني +967776358825',];
    }
